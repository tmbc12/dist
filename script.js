...
const renderList = year => {
  // Update the URL to the local JSON file
  const url = '/data/data.json'; // Update the path accordingly

  emptyState();

  // Use fetch to read the local JSON file
  fetch(url)
    .then(response => {
      if (!response.ok) {
        throw new Error(`Failed to fetch local JSON file: ${response.statusText}`);
      }
    ...
    .then(data => {
      ...
      table.innerHTML = `
        ...
        <tbody>
          <!-- ... (you can add the rows dynamically based on the local data) ... -->
        </tbody>
      `;
      ...
      data.data.forEach((item, index) => {
        ...
        tr.innerHTML = `
          <td class="c-table__cell c-table__cell--place u-text--center"><span class="c-place">${index + 1}</span></td>
          <td class="c-table__cell c-table__cell--name">${item.name}<br><small style="opacity: .4;">${item.team}</small></td>
          <td class="c-table__cell c-table__cell--points u-text--right"><strong>${item.point}</strong></td>
        `;

        if (index === 0) {
          tr.querySelector('.c-place').classList.add('c-place--first');
          // Additional logic for the first place (similar to your existing code)
